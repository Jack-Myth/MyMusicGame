...
FString AFunctionLibrary::GetMusicName(FString MusicName)
{
	...
	TCHAR TC[1024] = { 0 };
	char MusicianA[1024];
	//FFileHelper::LoadANSITextFileToStrings(*(FPaths::ProjectContentDir() + "/MyMusicGame/MusicData/" + MusicName), NULL, FileC);
...
{
	//GEngine->AddOnScreenDebugMessage(-1, 10, FColor::Green, FPaths::ProjectContentDir() + "/MyMusicGame/MusicData/" + MusicFileName);
...
void AFunctionLibrary::GetVideoInfo(FString VideoPath, FGetVideoInfoDelegate Delegate)
{
	TSharedPtr<IHttpRequest> HRequest;
	HRequest = FHttpModule::Get().CreateRequest();
	HRequest->SetURL(TEXT("https://jackmyth.cn/GameInterface/MYMusicGame/VideoInfo/")+VideoPath + TEXT(".html"));
